...
local t_m = EntityGetInRadiusWithTag( x, y, radius, "mysterystone" )
...
if ( comp ~= nil ) and ( comp2 ~= nil ) then
    ...
    if m then
		if ( string.find( essences_list, "mystery" ) == nil ) then
			EntitySetComponentsWithTagEnabled( entity_id, "mystery", true )
			ComponentSetValue2( comp2, "image_file", "data/props_gfx/sun_small_greener.png" )
			essences_list = essences_list .. "mystery,"
			EntityLoad("data/entities/projectiles/deck/explosion_giga.xml", x, y)
			GameScreenshake( 30, x, y )
		end
	...
	if ( found == 4 ) then
		if ( ohno == false ) then
            ...
            if ( ohyes == false ) then
                ...
                EntityLoad("data/entities/items/pickup/sun/newsun_green.xml", x, y)
                GamePrintImportant( "SOMETHING WENT RIGHT...!", "EVERYTHING IS RIGHT HERE..." )
